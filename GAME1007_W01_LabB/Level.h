...
private:
	...
	virtual void setNumCollectibles(unsigned int num) { m_iNumCollectibles = num; }
public:
	// System functions
	Level(unsigned int id, std::string tilemapfile, std::string name, std::string coverimagefile, std::string unlockcode, unsigned int collectibles) : 
		DataSet(id, tilemapfile, name), m_strCoverImageFileName(coverimagefile), m_strUnlockCode(unlockcode), m_iNumCollectibles(collectibles), m_pTileMap(nullptr) {}
	~Level() { Clean(); }
	...
	virtual unsigned int getNumCollectibles() { return m_iNumCollectibles; }
