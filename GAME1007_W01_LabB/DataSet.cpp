	...
	for (unsigned int setId; pElement[0] != nullptr; pElement[0] = pElement[0]->NextSiblingElement(CHILD_WEAPONSET))
	{
		...
		if (setId == getId())
		{
			...
			for (unsigned int itemId; pElement[1] != nullptr; pElement[1] = pElement[1]->NextSiblingElement(CHILD_WEAPON))
			{
				...
				std::string name;
				float recoil, refireRate, reloadRate;
				unsigned int maxShots, maxReloads;
				...
				pElement[1]->QueryFloatAttribute(ATTRIBUTE_REFIRERATE, &refireRate);
				pElement[1]->QueryFloatAttribute(ATTRIBUTE_RELOADRATE, &reloadRate);
